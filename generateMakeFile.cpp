...
void generateBinaryForOtherApps(const std::string& appname, const bool windows, std::set<CppFile>& filenames, std::set<std::string>& c_filenames)
{
	std::cout << appname << ": ";
	...
	if(appname != "bitvanity")
	{		
		std::cout << appname << ".o ";
	}
	else
	{
		std::cout << "bitvanity_bitcoin.o ";		
	}
	
	std::cout << allObjectFiles(filenames, windows) << allCObjectFiles(c_filenames, windows) << oneObjectFile("currencySpecific_bitcoin_specific.cpp", windows) << std::endl;
	std::cout << "\t";
	if(windows)
	{
		std::cout << "i686-w64-mingw32-g++ --std=c++11 -pthread -static";
	}
	else
	{
		std::cout << "g++ --std=c++11";
...
void linkCoinExecutable(const std::string& coin, const bool windows, std::set<CppFile>& filenames, std::set<std::string>& c_filenames,
	const bool defaultCoin = false) 
{
	std::cout << (defaultCoin ? "bitcoindefault" : coin ) << ": ";
	...
	std::cout << " -o " << exeName(defaultCoin ? "bitcoindefault" : coin);
	if(windows)
	{
		std::cout << ".exe";
	}
...
int doTheWork(const std::string& param)
{
	...
	if(windows)
	{
		std::cout << "COMPILE=i686-w64-mingw32-g++ --std=c++11 -O2 -msse2 -I. -Isrc -Isrc/bitlib/bigintctl -Isrc/bitlib -Isrc/qrenc -Isrc/argon -Isrc/bitgen -DSHA2_USE_INTTYPES_H -DBYTE_ORDER=LITTLE_ENDIAN -static -c" << std::endl;
	}
	else
	{
		std::cout << "ifeq (\"$(MAKECMDGOALS)\",\"withoutbw\")" << std::endl;
		std::cout << "\tCOMPILE=g++ --std=c++11 -I. -Isrc -Isrc/bitlib/bigintctl -Isrc/bitlib -Isrc/qrenc -Isrc/argon -Isrc/bitgen -c" << std::endl;
		std::cout << "else" << std::endl;
		std::cout << "\tCOMPILE=g++ --std=c++11 -O2 -msse2 -I. -Isrc -Isrc/bitlib/bigintctl -Isrc/bitlib -Isrc/qrenc -Isrc/argon -Isrc/bitgen -c" << std::endl;
	...
	std::cout << "all: bitsig bitcry bitcalc bitcoindefault bitvanity ";
	...
	for(;;)
	{
		...
		if(filename == "bitsig.cpp" || filename == "bitcry.cpp" || filename == "bitcalc.cpp" || filename == "bitvanity_bitcoin.cpp")
		{
			continue;
		}
	...
	Directory vanityDir("src/vanity");
	for(;;)
	{
		std::string filename;
		if(!vanityDir.readEntry(filename))
		{
			break;
		}
		...
		if(endsWith(filename, ".cpp"))
		{
			CppFile myFile("vanity", filename);
			filenames.insert(myFile);
		}
	
	}

	Directory testDir("src/test");
	for(;;)
	{
		std::string filename;
		if(!testDir.readEntry(filename))
		{
			break;
		}
...
*/
		...
		++it)
	{
		...
		if(coin == "bitcoincash") //Bitcoin cash if the "bitgen" executable without ending
		{
...
//--------------------------------------------
	...
	generateBinaryForOtherApps("bitsig", windows, filenames, c_filenames);
	generateBinaryForOtherApps("bitcry", windows, filenames, c_filenames);
	generateBinaryForOtherApps("bitcalc", windows, filenames, c_filenames);
	generateBinaryForOtherApps("bitvanity", windows, filenames, c_filenames);
...
//--------------------------------------------
	...
	CppFile file_bitcoincash_slp("", "bitgen_bitcoincash_slp.cpp");	
	compileFilename.insert(file_bitcoincash_slp);
	...
	CppFile file_bitvanity("", "bitvanity_bitcoin.cpp");							
	compileFilename.insert(file_bitvanity);
...
std::cout << "\trm -f *.o *.exe obj/*.o objwin/*.o bitgen bitsig bitcry bitcalc bitgen-btc bitgen-ltc bitgen-doge bitgen-testnet bitgen-nmc bitgen-ppc bitgen-vtc bitgen-rdd bitgen-ftc bitgen-bch bitgen-slp bitvanity bitvanity-btc" << std::endl;
std::cout << std::endl;

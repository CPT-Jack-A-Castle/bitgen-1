...
public:	
	...
	void finally() const;
	void setAccountNameforFolder(const bool useFolder, const int newNum = 0);
	...
	int getWords() const { return numWords; }
	bool useConter() const { return counter; }

private:

	static void verifyNumWords(const int numWords);


	bool getName;			//Used internally
	bool getPasswordParam;	//Used internally
	bool getNumWords;		//Used internally

...
std::string PostscriptGenerator::createEncString(const PublicPostscriptInfo& pubInfo, const std::string& amount, const std::string& message)
{
	...
	std::string encStr(urlPrefix + pubInfo.pubkey.toString());
...
PsInfoFile
PostscriptGenerator::generateExtended(
	const PrivatePostscriptInfo& privInfo,
	const PublicExtendedPostscriptInfo& pubInfo
	) const
{

	const std::string psPublic = postscriptPagePublicExtended(pubInfo);

	const std::string psPrivate = generatePrivateExtended(true, privInfo, pubInfo);

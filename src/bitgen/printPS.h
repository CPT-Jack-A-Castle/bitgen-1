...
{
public:
	PublicPostscriptInfo(const PubAddress& inPubkey, const std::string& inThetime, 
		const std::string& inAccountName,
		const bool inDisplayHashOfTime = false) : 
		pubkey(inPubkey/*.toString()*/), thetime(inThetime), accountName(inAccountName), displayHashOfTime(inDisplayHashOfTime) {}
	
	const PubAddress /*std::string*/ pubkey;
	const std::string thetime;
	const std::string accountName;	
	const bool displayHashOfTime;
};



class PublicExtendedPostscriptInfo
{
public:
	PublicExtendedPostscriptInfo(const std::string & inPubkey, const std::string& inThetime, const std::string& inAccountName,
		const bool inDisplayHashOfTime = false) : 
		pubkey(inPubkey), thetime(inThetime), accountName(inAccountName), displayHashOfTime(inDisplayHashOfTime) {}
...
public:
	PrivatePostscriptInfo(const std::string& inPrivkey, const std::string& inMnemonic, 
		...
		const bool inIsPad = false, const int inPadNum = 0, const int inTotalPads = 0,
		const BigInt<256>& inStaticXor = BigInt<256>()) : 
			privkey(inPrivkey), mnemonic(inMnemonic), generationMethod(inGenerationMethod), 
			compressed(inCompressed), generationExtra(inGenerationExtra), isPad(inIsPad),
			padNum(inPadNum), totalPads(inTotalPads), staticXor(inStaticXor) {}

	bool hasBip39Mnemonic() const
	{
		//Todo: Remove hardcoded string
		if(generationMethod == "Random with HD first address derivation")
		{
			return true;
		}
...
std::string generateStaticXor(const std::string& thetime, const BigInt<256>& staticXor) const;
std::string staticXorInfo(const std::string& thetime, const BigInt<256>& staticXor) const;
...
PsInfoFile generateExtended(const PrivatePostscriptInfo& privInfo, const PublicExtendedPostscriptInfo& pubInfo) const;


std::string postscriptPagePublic(const PublicPostscriptInfo& pubInfo) const;
std::string postscriptPagePublicExtended(const PublicExtendedPostscriptInfo& pubInfo) const;
...
static std::string generateQRPostscriptWithText(const QRInfo& qrinfo, 
...
static QRInfo qrEncodeAddress(const std::string& pubkeyVal, const std::string& accountName, const bool useAlphaNumeric = false);
...
std::string generatePrivateExtended(const bool isPrivate, const PrivatePostscriptInfo& privInfo, const PublicExtendedPostscriptInfo& pubInfo) const;
...
std::string publicMnemonic(const PublicExtendedPostscriptInfo& pubInfo, const bool useSmallDescription, 
...
std::string publicPartExtended(const PublicExtendedPostscriptInfo& pubInfo, const bool useSmallText) const;
...
static std::string createPrivateEncString(const PrivatePostscriptInfo& privInfo);
	...
	bool isElectrum() const { return (addrType == ADDRTYPE_ELECTRUM); }

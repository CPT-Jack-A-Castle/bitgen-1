...
BigInt<256> parseHex256Bit(const std::string& hexStr)
{
	if(hexStr.size() != 64)
	{
		std::string errMsg("Hex number must be 64 hex characters, got: ");
		errMsg += intToString(hexStr.size());
		errMsg += " characters";
	...
	const std::string privKeyStrUpper = toUpper(hexStr);

...
void signMessageSchnorr(const std::vector<std::string>& args)
{	
	if(args.size() < 2)
	{
		std::cout << "Usage: bitsig signSchnorr messageFile [priveateAlias]" << std::endl;
		return;			
	}
	...
	std::cout << "Signing with Schnorr for message file: " << messageFile << std::endl;

	
	SimpleFile theFile(messageFile);

	const PrivKey pk = getPrivKeyForSigningSchnorr();
	...
	const Pkif pkif = pubk.getPkif();

		
	const std::string sigResult = createSignatureContentSchnorr(rs, pkif);
			
	const std::string outname = messageFile + ".sbitsig";
...
void verifyFile(const std::vector<std::string>& args)
{
	...
	const std::string signedContent = SignatureImplementationExtra::verifySignatureForFile(signedFile, pubadd, alias, verifiedAddress, false);
...
bool checkIfKnownPkif(const std::list<Pkif>& allPkif, const Pkif& pkif)
{
	for(const Pkif& knownPkif : allPkif)
	{
		if(knownPkif == pkif)
		{
			std::cout << "This is a known pkif" << std::endl;
			return true;
		}
	}

	std::cout << "Not a known pkif" << std::endl;
	return false;
}



void verifyMessageSchnorr(const std::vector<std::string>& args)
{
	std::cout << "Verifying schnorr message signature" << std::endl;
		
	if(args.size() < 2)
	{
		std::cout << "Usage: bitsig verifySchnorr signedFile [bitsigFile] [publicKey]" << std::endl;
		throw std::string("Missing argument");
		//return;			
	}
	...
	//Check that it end with ".bitsig"		
	const std::string ending = signFile.substr(signFile.size() - 8);
		
	if(ending != ".sbitsig")
	{
		std::cout << "The signature file must end with .sbitsig, this ends with " << ending << std::endl;
		throw std::string("Incorrect signature file");
	}

	const SchnorrFileData fileData = parseSignFileSchnorr(signFile);
	...
	//Todo: Support alias checking
	const std::list<Pkif> allPkif = BitsigConfig::getAllPkif();

	const bool isKnownPkif = checkIfKnownPkif(allPkif, fileData.pkif);

	if(!isKnownPkif)
	{
		std::cout << "Unknown pkif: " << std::endl;
		return;
	}


	const PubKey pubkey = fileData.pkif.getPubKey();
	const ECPoint pubPoint = pubkey.point;

	const bool signOk = SignatureImplementation::verifySignSchnorr(fileData.rs, pubPoint, hashNum);
	if(signOk)
	{
		std::cout << std::endl;
		std::cout << std::endl;
		std::cout << "Signature verified OK for: " << fileData.pkif.toString() << std::endl;
		return;
	}
...
{			
	...
	else if(firstArg == "signSchnorr")
	{
		signMessageSchnorr(args);
	}
	else if(firstArg == "verifySchnorr")
	{
		verifyMessageSchnorr(args);
	}	

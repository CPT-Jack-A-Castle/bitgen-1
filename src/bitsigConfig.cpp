...
PrivKey getPrivKeyForSigningSchnorr()
{
	const PrivKey privateKey = BitsigConfig::getDefaultPrivkeySchnorr();			

	
	//BitcoinEllipticCurve ec;			
	//const PubKey pubKey = privateKey.getPubkey(ec);
	//const Pkif pkif = pubKey.getPkif();	
	//std::cout << "No key specified, using the default key: " << pkif.toString() << std::endl;			
...
std::list<Pkif> BitsigConfig::getAllPkif()
{		
	std::list<Pkif> ret;
...
PrivKey BitsigConfig::getDefaultPrivkeySchnorr()
{
	const PubAddress defaultPubAddr = getPubForDefaultPriv();
	const std::string defaultPub = defaultPubAddr.toString();
